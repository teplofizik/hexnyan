...
{
    // Padding
    class PW : BasicElement
    {
        private int FieldSize = 0;

        public PW(int Size, uint Value)
        {
            FieldSize = Size;
            ...
            for (int i = 0; i < FieldSize; i++)
            {
                ...
                InternalData[i * 2 + 2] = Convert.ToByte((Value >> 16) & 0xFF);
                InternalData[i * 2 + 3] = Convert.ToByte((Value >> 24) & 0xFF);
            }
        }
        
        public PW(int Size, uint Value, uint[] Values)
        {
            FieldSize = Size;
            ...
            for (int i = 0; i < FieldSize; i++)
            {
                ...
                InternalData[i * 2 + 2] = Convert.ToByte((Val >> 16) & 0xFF);
                InternalData[i * 2 + 3] = Convert.ToByte((Val >> 24) & 0xFF);
            }
        }
